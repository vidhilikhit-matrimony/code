...
const INITIAL_FORM = {
    ...
    sendersInfo: '', expectations: '', gana: '', charana: ''
};
        ...
        const fetchExistingProfile = async () => {
            try {
                ...
                if (response.success && response.data) {
                    ...
                    toast.error(response.message || 'Failed to fetch existing profile profile data');
                }
            } catch (err) {
                // If it's a 404, the user just doesn't have a profile yet — stay in create mode
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                const params = adminUserId ? `?adminUserId=${adminUserId}` : '';

                // Clear dirty flag before navigating so warning doesn't trigger
                setHasUnsavedChanges(false);

                // Slightly wait so state updates don't block navigation
                setTimeout(() => {
                    if (adminUserId) {
                        navigate('/admin/dashboard');
                    } else if (!isEditMode) {
                        navigate('/profiles'); // Force re-fetch user after first creation
        ...
        switch (step) {
                            ...
                            {existingGallery.length + galleryFiles.length < 2 && (
                                <label className="w-20 h-20 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 flex flex-col items-center justify-center cursor-pointer hover:border-primary-500 transition-colors">
                                    ...
                                    <input
                                        ...
                                        onChangeCapture={handleGalleryChange}
                                        className="hidden"
                                    />
                                </label>
                            )}
            ...
            case 2: return (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    {formData.caste === 'Brahmin' && (
                        <>
                            <div className="space-y-1.5">
                                <label htmlFor="gana" className="label flex items-center gap-1.5">
                                    <Heart className="w-3.5 h-3.5 text-primary-500" />
                                    Gana
                                </label>
                                <CustomSelect
                                    ...
                                    onChange={handleChange}
                                    options={[
                                        { value: 'Deva Gana', label: 'Deva Gana' },
                                        { value: 'Manushya Gana', label: 'Manushya Gana' },
                                        { value: 'Rakshasa Gana', label: 'Rakshasa Gana' }
                                    ]}
            ...
            case 4: return (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <FormField label="Assets" name="assets" type="textarea" value={formData.assets} onChange={handleChange} required placeholder="e.g. House, Car" note="Add None if no assets" />
                </div>
            );
            case 5: return (
