...
function SuitableMatchesGrid({ onViewMore }) {
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await getAllProfiles({ limit: 16 });
...
const Home = () => {
    ...
    // Handle global visitor tracking per tab session
    useEffect(() => {
        ...
        (async () => {
            try {
                const SESSION_VISIT_KEY = 'vl_has_visited_session';
                const hasVisitedThisSession = sessionStorage.getItem(SESSION_VISIT_KEY);

                let res;
                // Treat as new visit if no session flag exists
                if (!hasVisitedThisSession) {
                    sessionStorage.setItem(SESSION_VISIT_KEY, 'true');
                    res = await api.post('/stats/visit');
                } else {
                        ...
                        <div className="hidden lg:flex items-center gap-8">
                            <nav className="flex items-center gap-6 text-[13px] font-bold text-slate-600 uppercase tracking-wider">
                                <div className="flex items-center gap-2">
                                    <span className="bg-slate-50 border border-slate-100 text-slate-500 font-bold px-2 py-0.5 rounded-full text-[10px] tracking-wider flex items-center gap-1 shadow-sm">
                    ...
                    {mobileMenuOpen && (
                        ...
                        >
                            <div className="px-4 py-6 space-y-4">
                                <div className="flex items-center justify-between bg-slate-50 -mx-4 px-4 py-2 my-2 border-y border-slate-100">
                                    <span className="bg-white border border-slate-200 text-slate-500 font-bold px-2.5 py-1 rounded-full text-xs flex items-center gap-1.5 shadow-sm">
                                        <Activity className="w-3.5 h-3.5 text-primary-500" />
                                        {totalVisitors.toLocaleString()} Visitors
                                    </span>
                                    <button onClick={() => navigate('/about-us')} className="block text-right font-bold text-slate-800">About Us</button>
                                </div>
